...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!$temErros) {
        ...
        if ($post && !empty($post['imagem'])) {
            $caminhoAntigo = __DIR__ . "/../../../uploads/users/" . $post['imagem'];
        ...
        if (move_uploaded_file($imagem["tmp_name"], $uploadPath)) {
            $stmt = $conn->prepare("UPDATE usuario SET imagem = ?, atualizado_em = current_timestamp() WHERE id = ?");
            $stmt->bind_param("si", $novoNome, $idusuario);

            if ($stmt->execute()) {
                setFlashMessage("Imagem do usuário atualizada com sucesso.", "success", "fas fa-check-circle");
